...
class Firewall(Component):
    ...
    def _on_http_server_finished(self, data, stream_id, pid):
        ...
        self.send_event({"kind": "http_download", "hash": payload_hash,
                    "pid": pid, "serveraddr": stream_id.src, "port": stream_id.sport})

    def _detect_deep_packet_analysis(self, packet, scapy_packet):
        if scapy_packet.haslayer(TCP):
            self._stream_tracker.process_packet(
                scapy_packet, packet.is_outbound)
