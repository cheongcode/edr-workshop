...
class Stream:
    ...
    def process_packet(self, packet, is_outgoing):
        ...
        # Save packet payload according to its sequence
        if packet.haslayer(Raw):
            self._segments[packet[TCP].seq - self._initial_seq] = packet[Raw].load
        
        # Check for connection closure (FIN/RST flags)
...
class StreamTracker:
    def __init__(self):
        # The streams we are currently tracking ({StreamId: (Stream)})
        self._tcp_streams = {}

        # Filters for tracking ({'filter', 'on_finish', 'once''})
        self._filters = []
    ...
    def start_tracking(self, stream_id_filter, on_finish, once=False):
        ...
        self._filters.append({  "filter": stream_id_filter,
                                "on_finish": on_finish,
                                "once": once})
