...
def handle_exe_file(event):
    ...
    if event['hash'] in MALWARES_SHA1:
        ...
        if event['path'] in origins:
            origin = origins[event['path']]
            event.update(origin)

            p.kill_process(origin['downloader_pid'])
            # To add origin_str for the alert
...
def handle_zip_file(event):
    ...
    if zip_hash in origins:
        origin = origins[zip_hash]
        event.update(origin)
        p.kill_process(origin['downloader_pid'])
        alert(  event,
...
def handle_user_added(event):
    ...
    # User addition is suspicious - let's see if any of the parent process is from the internet
    while True:
        ...
        origin = origins.get(file_hash, None)
        if origin:
            p = Prevention()
            ...
            proc_tree.reverse()
            event['proc_tree_str'] = ''.join(['\n{}{} ({})'.format(' '*4*i, p[1], p[0]) for i, p in enumerate(proc_tree)])
            alert(  event,
...
def handle_rundll32_exec(event):
    origin = origins.get(hash_file(event['path']), None)
    if not origin:
        return
...
event_handlers = {
    ...
    'http_download': handle_http_download,
    'curl_download': handle_curl_download,
    'camouflaged_exe': handle_camouflaged_exe,
    'user_added': handle_user_added,
    'new_startup_command': handle_new_startup_command,
    'new_service': handle_new_service,
    'rundll32_exec': handle_rundll32_exec
}

def main():
    logging.info("Starting components")
    event_queue = queue.Queue()
    components = [  FileMon(event_queue), 
